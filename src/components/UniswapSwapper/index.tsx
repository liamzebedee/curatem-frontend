...
const initialise = (
    ...
    dispatch(calculate(pool, erc20Template, state.amountIn));
};
...
const executeSwap = (pool: ethers.Contract): AppThunk => async (dispatch, getState) => {
    ...
    try {
        await pool.swapExactAmountIn(tokenIn, tokenAmountIn, tokenOut, minAmountOut, maxPrice.toString());
    } catch (ex) {
        dispatch(executeSwapError({ error: ex.toString() }));
    }
};

const executeApprove = (pool: ethers.Contract, tokenTemplate: ethers.Contract): AppThunk => async (
    dispatch,
    getState,
) => {
    const state = selectBalancerSwapperState(getState());
...
export function UniswapSwapper(props: any) {
    ...
    useEffect(() => {
        if (contract != null) dispatch(initialise(contract, erc20, tokens));
    }, [dispatch, contract, erc20, tokens]);

    const onNumberInputChange1 = useCallback(
        () =>
            debounce(function onNumberInputChange1(valueString: string) {
                if (valueString == '') return;

                const parse = (val: string) => val.replace(/^\$/, '');
                const amountEtherFormat = parse(valueString);
                dispatch(calculate(contract, erc20, toWei(amountEtherFormat).toString()));
            }, 350),
    ...
    function selectToken(ev: React.ChangeEvent<HTMLSelectElement>) {
        ...
        dispatch(calculate(contract, erc20, state.amountIn));
    }
